...
data = dict(
    samples_per_gpu=1, workers_per_gpu=0, train=train, val=test, test=test)
...
model = dict(
    ...
    bbox_head=dict(
        type='SDMGRHead', visual_dim=16, num_chars=92, num_classes=18),
    visual_modality=True,
    train_cfg=None,
    test_cfg=None,
    class_list=f'{data_root}/class_list.txt')

optimizer = dict(type='Adam', weight_decay=0.0001,lr=0.001)
